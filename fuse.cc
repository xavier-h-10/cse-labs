...
chfs_client::status getattr(chfs_client::inum inum, struct stat &st) {
    ...
    } else if(chfs->isdir(inum))
    {
        ...
        st.st_mode = S_IFLNK | 0777;   //权限 -rwx
        st.st_nlink = 1;
        st.st_atime = info.atime;
        st.st_mtime = info.mtime;
        st.st_ctime = info.ctime;
        st.st_size = info.size;
        printf("   getattr -> %lu %lu %lu %llu\n", info.atime, info.mtime, info.ctime, info.size);
    }
...
//    std::cout<<"getattr called inum="<<inum<<std::endl;
    ret = getattr(inum, st);

//    std::cout<<"getattr completed size="<<st.st_size<<std::endl;
    if (ret != chfs_client::OK) {
...
void fuseserver_setattr(fuse_req_t req, fuse_ino_t ino, struct stat *attr,
    ...
    if (FUSE_SET_ATTR_SIZE & to_set) {
        ...
        //TODO: Implement: If the new size is bigger than the current file size, fill the new bytes with '\0'.
        struct stat st;
        chfs_client::status ret;
...
void fuseserver_read(fuse_req_t req, fuse_ino_t ino, size_t size,
    ...
    chfs_client::status ret;
    std::string buf;
    ret = chfs->read(ino, size, off, buf);
    if (ret != chfs_client::OK) {
        fuse_reply_err(req, ENOENT);
        return;
    }
...
void fuseserver_write(fuse_req_t req, fuse_ino_t ino,
    ...
    ret = chfs->write(ino, size, off, buf, bytes_written);
    if (ret != chfs_client::OK) {
        fuse_reply_err(req, ENOENT);
        return;
    }
...
//
chfs_client::status fuseserver_createhelper(fuse_ino_t parent, const char *name,
...
//    std::cout << "fuseserver_lookup called parent=" << parent << " name=" << name << std::endl;
    chfs->lookup(parent, name, found, ino);
//    std::cout << "fuseserver_lookup parent=" << parent << " name=" << name << "found="<<found<<std::endl;
...
//    std::cout << "fuseserver_mkdir called" << std::endl;
    // Change the above line to "#if 1", and your code goes here
    int ret = fuseserver_createhelper(parent, name, mode, &e, extent_protocol::T_DIR);
    if (ret == chfs_client::OK) {
//        e.ino=2;
//        std::cout << "fuseserver_mkdir chfs_client::OK e.ino=" << e.ino << std::endl;
        fuse_reply_entry(req, &e);
...
void fuseserver_readlink(fuse_req_t req, fuse_ino_t ino) {
    int r;
    std::string buf;
//    std::cout << "fuseserver_readlink called ino=" << ino << std::endl;
    r = chfs->readlink(ino, buf);
    if (r == chfs_client::OK) {
//        std::cout << "fuseserver_readlink completed buf=" << buf << std::endl;
...
//    std::cout << "fuseserver_symlink called link=" << link << " name=" << name << std::endl;
    r = chfs->symlink(parent, name, link, id);
    if (r == chfs_client::OK) {
        e.ino = id;
        getattr(id, e.attr);
//        std::cout << "fuseserver_symlink completed ino=" << id << std::endl;
        fuse_reply_entry(req, &e);
